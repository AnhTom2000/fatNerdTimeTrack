...
`event_title` VARCHAR(30) NOT NULL COMMENT '事件标题',
`event_description` VARCHAR(255) NOT NULL DEFAULT '0' COMMENT '事件描述,0表示没有描述',
`user_id` BIGINT(20) UNSIGNED NOT NULL COMMENT '用户表主键',
`priority_id` TINYINT(1) UNSIGNED NOT NULL COMMENT '优先级表主键,代表事件选择了哪个优先级',
`finished` BIT UNSIGNED NOT NULL COMMENT '是否已完成',
`time` TIMESTAMP NOT NULL  DEFAULT '0000-00-00' COMMENT '事件开始时间，可以没有时间',
